...
async function startConsumer() {
    try {
        ...
        channel.consume(queue, async (message) => {
            ...
            const { intento, libreria } = JSON.parse(message.content.toString()); //Extrae el contenido del mensaje y lo convierte en un objeto JSON

            const [res, id, error] = await services(libreria) // Llama a la función services y le pasa el nombre de la colección
            if (res) {
                ...
                channel.publish(exchange, config.routingKeyNext, Buffer.from(JSON.stringify({ id_chat: id, libreria, intento: 1 })));
                console.log(`📤 Enviando mensaje a ${config.routingKeyNext}`);
            } else {
                if (intento < 3) {
                    console.log(`🔄 Reintentando mensaje (${intento + 1}/3)...`);
                    channel.publish(exchange, config.routingKey, Buffer.from(JSON.stringify({ intento: intento + 1, libreria })), { expiration: 5000 });
                } else {
        ...
        const e = new CustomError(`Error desconocido en la creación del documento. ${error.message}`, { collecion });
        console.log(`💀 Enviando mensaje a ${config.queueErrors}`)
        channel.ack(message)
